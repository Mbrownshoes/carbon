...
function expandImageData(compressed, width, height) {
    ...
    var xScale = d3.scaleLinear().domain([0, compressed.width]).range([0, scaledWidth]);
    var yScale = d3.scaleLinear().domain([0, compressed.height]).range([yTranslate, scaledHeight + yTranslate]);
    var hue = 205;
    var saturation = .74;
...
function loadData(width, height) {
    return new Promise(function(resolve, reject) {
        ...
        d3.queue().defer(citiesCsv, "sampled_cities_data.csv").defer(d3.json, "img.json").await(function(err, citiesData, imgData) {
            if (err) {
...
function barsLayout(points, width, height, citiesData) {
    ...
    var byContinent = d3.nest().key(function(d) {
        return d.continent
    }).entries(citiesData).filter(function(d) {
...
function swarmLayout(points, width, height, citiesData) {
    ...
    var rng = d3.randomNormal(0, .3);
    points.forEach(function(d, i) {
        d.y = .75 * rng() * height + height / 2
    })
}
...
function phyllotaxisLayout(points, pointWidth, xOffset, yOffset, citiesData) {
    ...
    colorDataByContinent(points, citiesData);
    var sortData = citiesData.map(function(city, index) {
        return {
            index: index,
